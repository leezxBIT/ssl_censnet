...
from utils import accuracy, RMSELoss
from utils import load_data, PairwiseDistance
...
def experimenter(data_name='cora', train_ratio=0.03, cuda=True, random_seed=42, hidden=16, dropout_ratio=0.5,
                 learning_rate=0.01, weight_decay=5e-4, num_epochs=65, early_stopping=30, task='classification', public_splitting=False):
    ...
    if data_name in ['cora', 'citeseer', 'pubmed']:
        ...
        ssl_agent_n = PairwiseDistance(adj, features, nhid=args.hidden, cuda=cuda, node=True)
        ssl_agent_e = PairwiseDistance(eadj, edge_features, nhid=args.hidden, cuda=cuda, node=False)
    ...
    def train(epoch):
        ...
        model.train()
        optimizer.zero_grad()
        output, n_feature, e_feature = model(features, edge_features, eadj, adj, Tmat, task)
    ...
    def pretrain(epoch):
        ...
        model.train()
        optimizer.zero_grad()
        output, node_f, edge_f = model(features, edge_features, eadj, adj, Tmat, task)
...
if __name__ == '__main__':
    ...
    file_path = "./results/cora_1pct/" + args.saved_name
    mean = 0
