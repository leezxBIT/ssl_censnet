...
def preprocess_adj_2(adj, device):
...
class PairwiseDistance:
    def __init__(self, adj, features, nhid, cuda, node=True):
        self.adj = adj
        ...
        self.nclass = 4
        if node:
            self.linear = nn.Linear(nhid, self.nclass)
        else:
            self.linear = nn.Linear(3, self.nclass)
        if cuda:
    ...
    def classification_loss(self, embeddings):
        if self.pseudo_labels is None:
            ...
            self.pseudo_labels = self.agent.get_label()
        ...
        if self.cuda:
            ...
            self.pseudo_labels = self.pseudo_labels.cuda()
        loss = loss(embeddings, self.pseudo_labels[node_pairs])
...
class NodeDistance:
    ...
    def __init__(self, adj, nclass=4):
        ...
        """
        self.adj = sp.coo_matrix(adj.to_dense().numpy(), dtype=sp.coo_matrix)
